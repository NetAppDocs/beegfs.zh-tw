...
WARNING: 請勿指定個別資源或資源群組。請務必指定您要重新部署BeeGFS服務的監視器名稱。例如、若要將BeeGFS管理服務移至ictad22h02、請執行： `pcs resource move mgmt-monitor ictad22h02`。您可以重複此程序、將一或多個服務移出偏好的節點。使用進行驗證 `pcs status` 這些服務已在新節點上重新部署/啟動。
