...
點擊 link:https://github.com/NetApp/beegfs/blob/master/getting_started/beegfs_on_netapp/gen2/H2_DAC_IB_A2_EF600/beegfs_cluster_building_blocks/beegfs_ha_building_block_cluster_1/host_vars/beegfs_01.yml["請按這裡"^] 查看代表單一檔案節點的完整庫存檔案範例。
...
NVIDIA ConnectX-Virtual Protocol Interconnect&reg; （VPI） 介面卡支援將 InfiniBand 和乙太網路作為傳輸階層。模式之間的切換並非自動交涉，必須使用  `mstflint` 中包含的  `mstconfig` 工具進行設定，這是一個屬於 link:https://network.nvidia.com/products/adapter-software/firmware-tools/["NVIDIA Firmare 工具（ MFT ）"^] 的開放原始碼套件。變更介面卡的模式只需要執行一次。這可以手動完成，也可以作為使用清單中  `eseries-[ib|ib_iser|ipoib|nvme_ib|nvme_roce|roce]_interfaces:` 區段設定之任何介面的一部分包含在 Ansible 清單中，以便自動進行檢查/套用。
