...
. 使用 link:beegfs-technology-requirements.html#file-node-requirements["1M InfiniBand HDR 200GB 直接連接銅線"] 連接區塊和檔案節點，使它們與下圖所示的拓撲相符。
+
...
. 使用適用於您的 InfiniBand 儲存交換器的 link:beegfs-technology-requirements.html#storage-network-switch-requirements["2 公尺 InfiniBand 纜線"]，將檔案節點上的剩餘 InfiniBand 連接埠連接到儲存網路的 InfiniBand 交換器。
+
