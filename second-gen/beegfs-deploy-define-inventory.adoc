...
NOTE: Volume 大小以整個儲存資源池（也稱為 Volume 群組）的百分比指定。NetApp 強烈建議您在每個資源池預留一些可用容量，以便為 SSD 過度資源配置留出空間（如需詳細資訊，請參閱 https://www.netapp.com/pdf.html?item=/media/17009-tr4800.pdf["NetApp EF600陣列簡介"^]）。儲存資源池 `beegfs_m1_m2_m5_m6`也會為管理服務分配資源池容量的 1%。因此，對於儲存資源池 `beegfs_m1_m2_m5_m6`中的中繼資料 Volume，當使用 1.92TB 或 3.84TB 磁碟機時，請將此值設為 `21.25`；對於 7.65TB 磁碟機，請將此值設為 `22.25`；對於 15.3TB 磁碟機，請將此值設為 `23.75`。
